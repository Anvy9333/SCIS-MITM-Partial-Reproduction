// AES-128 encryption and AES-MMO compression 
#include "aes.h"
#include <string.h>

// S-box 
static const uint8_t SBOX[256] = {
    ,
};

// GF(2^8) multiply
static inline uint8_t xtime(uint8_t a) {
    return (a & 0x80) ? ((a << 1) ^ 0x1b) : (a << 1);
}
static inline uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        uint8_t hi = a & 0x80;
        a <<= 1;
        if (hi) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

// Round constants 
static const uint8_t RCON[10] = {
    0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

// Key schedule 
static void key_schedule(const uint8_t key[16], uint8_t rk[11][16]) {
    memcpy(rk[0], key, 16);
    for (int r = 1; r <= 10; r++) {
        uint8_t *prev = rk[r-1];
        uint8_t *cur  = rk[r];
        // First word 
        cur[0]  = prev[0]  ^ SBOX[prev[13]] ^ RCON[r-1];
        cur[1]  = prev[1]  ^ SBOX[prev[14]];
        cur[2]  = prev[2]  ^ SBOX[prev[15]];
        cur[3]  = prev[3]  ^ SBOX[prev[12]];
        // Remaining words
        for (int w = 1; w < 4; w++) {
            cur[w*4+0] = prev[w*4+0] ^ cur[(w-1)*4+0];
            cur[w*4+1] = prev[w*4+1] ^ cur[(w-1)*4+1];
            cur[w*4+2] = prev[w*4+2] ^ cur[(w-1)*4+2];
            cur[w*4+3] = prev[w*4+3] ^ cur[(w-1)*4+3];
        }
    }
}

// SubBytes 
static void sub_bytes(uint8_t s[16]) {
    for (int i = 0; i < 16; i++) s[i] = SBOX[s[i]];
}

// ShiftRows 
static void shift_rows(uint8_t s[16]) {
    uint8_t t;
    // Row 1: left shift by 1 
    t=s[1]; s[1]=s[5]; s[5]=s[9]; s[9]=s[13]; s[13]=t;
    // Row 2: left shift by 2 
    t=s[2]; s[2]=s[10]; s[10]=t;
    t=s[6]; s[6]=s[14]; s[14]=t;
    // Row 3: left shift by 3 
    t=s[15]; s[15]=s[11]; s[11]=s[7]; s[7]=s[3]; s[3]=t;
}

// MixColumns 
static void mix_columns(uint8_t s[16]) {
    for (int c = 0; c < 4; c++) {
        uint8_t a0=s[c*4], a1=s[c*4+1], a2=s[c*4+2], a3=s[c*4+3];
        s[c*4+0] = gmul(0x02,a0)^gmul(0x03,a1)^a2^a3;
        s[c*4+1] = a0^gmul(0x02,a1)^gmul(0x03,a2)^a3;
        s[c*4+2] = a0^a1^gmul(0x02,a2)^gmul(0x03,a3);
        s[c*4+3] = gmul(0x03,a0)^a1^a2^gmul(0x02,a3);
    }
}

// AddRoundKey 
static void add_round_key(uint8_t s[16], const uint8_t rk[16]) {
    for (int i = 0; i < 16; i++) s[i] ^= rk[i];
}

// AES-128 encrypt 
void aes128_encrypt(const uint8_t key[16], const uint8_t pt[16], uint8_t ct[16]) {
    uint8_t rk[11][16];
    key_schedule(key, rk);
    memcpy(ct, pt, 16);
    add_round_key(ct, rk[0]);
    for (int r = 1; r <= 9; r++) {
        sub_bytes(ct);
        shift_rows(ct);
        mix_columns(ct);
        add_round_key(ct, rk[r]);
    }
    sub_bytes(ct);
    shift_rows(ct);
    add_round_key(ct, rk[10]);
}

// AES-MMO compression function                   
void aes_mmo(const uint8_t msg[16], const uint8_t cv[16], uint8_t out[16]) {
    aes128_encrypt(msg, cv, out);
    for (int i = 0; i < 16; i++) out[i] ^= cv[i];
}
